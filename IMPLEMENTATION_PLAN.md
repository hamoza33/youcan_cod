...
- VPS SSH access method/password/key and sudo/deployment permissions for `deploy-user@YOUR_VPS_IP`.
- DNS confirmation that `app.example.com` points to the VPS.
...
- Caddy reverse proxy for `app.example.com`.
...
- Caddyfile template for `app.example.com`.
