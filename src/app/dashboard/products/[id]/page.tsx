...
import { ExternalProductButton, VisibilityToggle } from '@/components/product-bulk-controls';
import { prisma } from '@/lib/db';
import { updateProductAction } from '@/app/dashboard/products/actions';
import { formatDate } from '@/lib/utils';
import { getRuntimeSettings, settingString } from '@/lib/settings/runtime';
  ...
  return (
    <>
      <PageHeader
        ...
        description="Edit price, visibility, category, SEO content, and review all imported images before re-importing to YouCan or Google Merchant Center."
        actions={
      ...
      <div className="mb-6 rounded-3xl border border-slate-200 bg-white p-5 shadow-soft">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">{product.imageUrls.length} image(s) stored for this product. Images below are the exact images sent to YouCan/GMC payloads.</p>
          </div>
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {product.imageUrls.map((url, index) => (
            <a key={`${url}-${index}`} href={url} target="_blank" rel="noreferrer" className="group block overflow-hidden rounded-3xl border border-slate-200 bg-slate-50">
              <div className="relative aspect-[4/3] bg-white">
                <Image src={url} alt={`${product.name} image ${index + 1}`} fill className="object-contain p-2 transition group-hover:scale-[1.02]" sizes="(max-width: 1280px) 50vw, 360px" />
              </div>
              <div className="flex items-center justify-between gap-2 px-3 py-2 text-xs text-slate-500">
                <span>Image {index + 1}</span>
                <span className="inline-flex items-center gap-1 font-semibold text-brand-600"><ExternalLink className="h-3 w-3" /> Open original</span>
              </div>
              <div className="break-all px-3 pb-3 font-mono text-[10px] leading-4 text-slate-400">{url}</div>
            </a>
          ))}
        ...
        <form action={updateProductAction} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-soft">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <label className="space-y-2 text-sm font-semibold text-slate-700">
              ...
              <span className="block text-xs font-normal text-slate-500">Saving queues a YouCan update so visibility changes are pushed directly.</span>
            </label>
          ...
          <button className="mt-6 rounded-2xl bg-brand-600 px-5 py-2.5 text-sm font-semibold text-white">Save changes and update YouCan</button>
        </form>
        ...
        <aside className="space-y-4">
          <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-soft">
            ...
            <dl className="mt-4 space-y-3 text-sm">
              <Row label="Country" value={product.country} />
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-soft">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              ...
              <Badge>{product.visibilityStatus}</Badge>
            </div>
