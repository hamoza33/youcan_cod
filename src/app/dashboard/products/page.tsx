...
import { CountryCode, GmcStatus, ImportStatus, Prisma, StockStatus } from '@prisma/client';
...
export default async function ProductsPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const country = enumFilter<CountryCode>(params.country, CountryCode) ?? CountryCode.SA;
  ...
  return (
    <>
      <PageHeader
        ...
        description="Manage COD Network KSA discovery, account-specific SKU mapping, YouCan imports, SEO status, stock, visibility, and Google Merchant Center submissions."
        actions={<ProductToolbar country={country} />}
      />

      <form className="mb-4 grid gap-3 rounded-3xl border border-slate-200 bg-white p-4 shadow-soft md:grid-cols-5">
        <FilterSelect name="country" label="All countries" values={Object.values(CountryCode)} defaultValue={country} />
        ...
        <table className="min-w-[1400px] w-full border-collapse text-left text-sm">
          ...
          <tbody className="divide-y divide-slate-100">
            {products.map((product) => (
              <tr key={product.id} className="align-top hover:bg-slate-50/60">
                ...
                <td className="px-4 py-3"><StatusBadge value={product.visibilityStatus} /></td>
                ...
                <td className="px-4 py-3"><RowActions productId={product.id} /></td>
              </tr>
            ))}
            {products.length === 0 ? (
              <tr>
                <td colSpan={14} className="px-4 py-12 text-center text-slate-500">
                  No products imported yet. Start with “Discover KSA products”.
                </td>
              </tr>
            ) : null}
          </tbody>
