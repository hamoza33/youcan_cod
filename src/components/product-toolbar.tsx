...
import { PlayCircle, RefreshCw, Search, Send, Sparkles, Tags, UploadCloud } from 'lucide-react';
import { CountryCode } from '@prisma/client';
import { triggerDiscoveryForCountries, triggerProcessBatch, triggerStockSyncForCountries, triggerYouCanCategorySync } from '@/app/dashboard/products/actions';
import { GCC_COUNTRY_CODES } from '@/lib/settings/registry';

export function ProductToolbar({
  enabledCountries = [CountryCode.SA],
  selectedCountry,
  visibleProductIds = [],
}: {
  ...
  const visibleLabel = visibleProductIds.length ? `visible ${visibleProductIds.length}` : 'visible products';
  ...
  return (
    <div className="flex flex-wrap gap-2">
      ...
      >
        <Search className="h-4 w-4" /> Discover {label} catalog
      </button>
      <button
        type="button"
        ...
        disabled={pending || visibleProductIds.length === 0}
        onClick={() => startTransition(() => triggerProcessBatch(visibleProductIds))}
      >
        <PlayCircle className="h-4 w-4" /> Process {visibleLabel}
      </button>
      <button
        type="button"
        className="inline-flex items-center gap-2 rounded-2xl border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50 disabled:opacity-60"
